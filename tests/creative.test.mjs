...
import {clipAlpha,opacityAt,scaleAt,effectAlpha,textPose,frozenClip} from '../dist/creative.js';
...
still.fadeIn=still.fadeOut=0;still.opacityKeyframes=[{time:0,value:0},{time:2,value:1},{time:4,value:0}];assert.equal(opacityAt(still,0,4),0);assert.equal(opacityAt(still,1,4),.5);assert.equal(opacityAt(still,2,4),1);assert.equal(opacityAt(still,3,4),.5);assert.equal(clipAlpha(row,4),.5);still.scale=1;still.scaleKeyframes=[{time:0,value:1},{time:2,value:2.5},{time:4,value:1}];assert.equal(scaleAt(still,0,4),1);assert.equal(scaleAt(still,1,4),1.75);assert.equal(scaleAt(still,2,4),2.5);assert.equal(scaleAt(still,3,4),1.75);
