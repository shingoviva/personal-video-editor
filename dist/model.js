...
export function sanitizeTexts(values=[]){return(Array.isArray(values)?values:[]).slice(0,20).map((t,i)=>{const start=clamp(+t.start||0,0,86400-1/60),end=clamp(Number.isFinite(+t.end)?+t.end:start+3,start+1/60,86400);return{id:String(t.id||`text-${i}`),text:String(t.text??'').slice(0,2000),font:['Sans','Serif','Mono'].includes(t.font)?t.font:'Sans',align:['left','center','right'].includes(t.align)?t.align:'center',weight:[400,500,600,700,800].includes(+t.weight)?+t.weight:600,color:/^#[0-9a-f]{6}$/i.test(t.color||'')?t.color:'#ffffff',box:['none','dark','light'].includes(t.box)?t.box:'none',outline:clamp(+t.outline||0,0,8),size:clamp(+t.size||48,8,300),x:clamp(Number.isFinite(+t.x)?+t.x:.5,0,1),y:clamp(Number.isFinite(+t.y)?+t.y:.85,0,1),opacity:clamp(Number.isFinite(+t.opacity)?+t.opacity:1,0,1),fadeIn:clamp(Number.isFinite(+t.fadeIn)?+t.fadeIn:+t.fade||0,0,3),fadeOut:clamp(Number.isFinite(+t.fadeOut)?+t.fadeOut:+t.fade||0,0,3),motion:['none','rise','slide-left'].includes(t.motion)?t.motion:'none',motionDuration:clamp(+t.motionDuration||.4,.05,2),start,end}})}
export function sanitize(p){if(p?.version!==1||!Array.isArray(p.clips)||!Array.isArray(p.media)||p.clips.length>200||p.media.length>200)throw Error('対応していないプロジェクト形式です。');for(const c of p.clips){if(c.start!==undefined){if(!Number.isFinite(c.start)||c.start<0||c.start>86400)throw Error('配置時刻が無効です。');c.layer=clamp(Math.round(c.layer||0),0,c.kind==='audio'?3:2)}if(c.gap){c.gap=clamp(c.gap,.01,86400);continue}if(!p.media.some(m=>m.id===c.media))throw Error('素材情報が欠けています。');if(!Number.isFinite(c.in)||!Number.isFinite(c.out)||c.in<0||c.out<=c.in)throw Error('IN / OUT が無効です。');if(c.freezeDuration){c.freezeDuration=clamp(c.freezeDuration,1/30,60);c.freezeAt=clamp(c.freezeAt??c.in,0,p.media.find(m=>m.id===c.media).duration-.00001)}c.speed=clamp(c.speed??1,.05,20);c.endSpeed=clamp(c.endSpeed??c.speed,.05,20);c.color={...colors(),...c.color};c.audio={volume:1,mute:false,fadeIn:0,fadeOut:0,...c.audio};c.curve=c.curve||'constant';if(c.timingBase){const b=c.timingBase;if(!Number.isFinite(b.in)||!Number.isFinite(b.out)||b.out<=b.in||b.in<0)throw Error('速度区間が無効です。');c.timingBase={in:b.in,out:b.out,speed:clamp(b.speed??1,.05,20),endSpeed:clamp(b.endSpeed??b.speed??1,.05,20),curve:b.curve||'constant'}}}if((p.audioClips||[]).length>200)throw Error('音声クリップは200個までです。');const audioClips=(p.audioClips||[]).map(c=>{if(!p.media.some(m=>m.id===c.media)||!Number.isFinite(c.in)||!Number.isFinite(c.out)||c.in<0||c.out<=c.in)throw Error('音声の範囲が無効です。');return{...c,kind:'audio',linked:c.sourceClip?c.linked!==false:false,speed:clamp(c.speed??1,.05,20),endSpeed:clamp(c.endSpeed??c.speed??1,.05,20),curve:c.curve||'constant',hold:clamp(c.hold||0,0,10),layer:clamp(Math.round(c.layer||0),0,3),start:clamp(c.start||0,0,86400),audio:{volume:1,mute:false,fadeIn:0,fadeOut:0,...c.audio}}});return{...project(),...p,audioClips,audioTracks:Array.from({length:4},(_,i)=>({...project().audioTracks[i],...p.audioTracks?.[i]})),analysis:p.analysis||[],effects:(p.effects||[]).slice(0,20).filter(e=>['flash','black-in','black-out'].includes(e.type)).map(e=>({...e,start:clamp(e.start,0,86400),duration:clamp(e.duration,1/60,30),strength:clamp(e.strength??1,0,1)})),texts:sanitizeTexts(p.texts),bgm:p.bgm||{volume:.3}}}
