...
def render_tracks(job,project,clips,work,total):
 import core
 result=[];tracks=project.get('audioTracks',[]);solo=any(t.get('solo') for t in tracks)
 for layer in range(4):
  ...
  folder=work/f'audio-track-{layer}';folder.mkdir()
  for n,c in enumerate(windows):
   ...
   for k,(a,b,s,length) in enumerate(pieces):
    ...
    job['operation']=f'A{layer+1} の音声を処理中';core.run(job,args,length,.9,0)
