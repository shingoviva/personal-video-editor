...
export function sequence(p){
 const key=p.clips.map(c=>[c.id,c.start,c.layer,c.in,c.out,c.speed,c.endSpeed,c.curve,c.hold,c.gap,JSON.stringify(c.timingBase)].join(':')).join('|');
 const cached=sequenceCache.get(p);if(cached?.key===key)return cached.rows;
 const ends=[0,0];const rows=p.clips.map(c=>{const layer=c.layer===1?1:0,d=timing(c).duration,start=Number.isFinite(c.start)?Math.max(0,c.start):ends[layer];ends[layer]=Math.max(ends[layer],start+d);return{clip:c,layer,start,end:start+d,duration:d}});
 sequenceCache.set(p,{key,rows});return rows;
}
export function total(p){return Math.max(0,...sequence(p).map(r=>r.end))}
export function locate(p,t){
 const rows=sequence(p),end=Math.max(0,...rows.map(r=>r.end));if(!rows.length)return;
...
export function compileTimeline(p){
 ...
 const prepared=spans.map(r=>({...r,start:r.start-r.offset,end:r.end,duration:r.originalDuration}));
...
export function pasteClip(p,source,start,layer=0){anchor(p);const c=structuredClone(source);c.id=uid();c.start=Math.max(0,start);c.layer=layer===1?1:0;p.clips.push(c);return c}
...
export function splitClip(c,source){
 if(c.gap||source<=c.in+.001||source>=c.out-.001)return null;
...
export function sanitize(p){if(p?.version!==1||!Array.isArray(p.clips)||!Array.isArray(p.media)||p.clips.length>200||p.media.length>200)throw Error('対応していないプロジェクト形式です。');for(const c of p.clips){if(c.start!==undefined){if(!Number.isFinite(c.start)||c.start<0||c.start>86400)throw Error('配置時刻が無効です。');c.layer=c.layer===1?1:0}if(c.gap){c.gap=clamp(c.gap,.01,86400);continue}if(!p.media.some(m=>m.id===c.media))throw Error('素材情報が欠けています。');if(!Number.isFinite(c.in)||!Number.isFinite(c.out)||c.in<0||c.out<=c.in)throw Error('IN / OUT が無効です。');c.speed=clamp(c.speed??1,.05,20);c.endSpeed=clamp(c.endSpeed??c.speed,.05,20);c.color={...colors(),...c.color};c.audio={volume:1,mute:false,fadeIn:0,fadeOut:0,...c.audio};c.curve=c.curve||'constant';if(c.timingBase){const b=c.timingBase;if(!Number.isFinite(b.in)||!Number.isFinite(b.out)||b.out<=b.in||b.in<0)throw Error('速度区間が無効です。');c.timingBase={in:b.in,out:b.out,speed:clamp(b.speed??1,.05,20),endSpeed:clamp(b.endSpeed??b.speed??1,.05,20),curve:b.curve||'constant'}}}return{...project(),...p,analysis:p.analysis||[],texts:p.texts||[],bgm:p.bgm||{volume:.3}}}
