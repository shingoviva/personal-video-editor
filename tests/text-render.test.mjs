...
assert.equal(text.outline,0);
assert.equal(text.size,300);
assert.equal(text.x,1);
assert.equal(text.y,0);
assert.ok(text.end>text.start);
assert.deepEqual(textStyle(text),{font:'Serif',align:'right',weight:800,color:'#f2c8b0',outline:0,box:'dark',letterSpacing:0,lineHeight:1.12});
