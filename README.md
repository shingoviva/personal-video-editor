...
画面はウインドウの高さに合わせ、素材／設定／タイムラインをそれぞれスクロール。タイムラインのトラック名と縦スクロールを同期し、編集ツールは固定します。素材・設定の⤢でパネルを広げ、もう一度押すと戻ります。iPhoneはキーボード表示時の有効高さにも合わせ、ビューアーがタイムラインを覆わない3段構成にします。基本操作を常時表示し、コピーなどは「その他」から開けます。
