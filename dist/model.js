...
export function total(p){return Math.max(0,...sequence(p).map(r=>r.end),...audioSequence(p).map(r=>r.end),...(p.effects||[]).map(e=>(+e.start||0)+(+e.duration||0)),...(p.texts||[]).map(t=>+t.end||0))}
...
export const MAX_TEXT_LAYERS=120;
const textColor=(value,fallback)=>/^#[0-9a-f]{6}$/i.test(value||'')?value:fallback;
export function sanitizeTexts(values=[]){return(Array.isArray(values)?values:[]).slice(0,MAX_TEXT_LAYERS).map((t,i)=>{const start=clamp(+t.start||0,0,86400-1/60),end=clamp(Number.isFinite(+t.end)?+t.end:start+3,start+1/60,86400),font=String(t.font||'Sans').replace(/["'`;{}]/g,'').slice(0,80)||'Sans',lineColors=(Array.isArray(t.lineColors)?t.lineColors:[]).slice(0,20).map(value=>textColor(value,'#ffffff'));return{id:String(t.id||`text-${i}`),layer:clamp(Math.round(t.layer||0),0,2),text:String(t.text??'').slice(0,2000),font,align:['left','center','right'].includes(t.align)?t.align:'center',weight:[400,500,600,700,800].includes(+t.weight)?+t.weight:600,color:textColor(t.color,'#ffffff'),lineColors,accentWords:String(t.accentWords??'').slice(0,240),accentColor:textColor(t.accentColor,'#ff817b'),outline:clamp(+t.outline||0,0,20),outlineColor:textColor(t.outlineColor,'#000000'),outerOutline:clamp(+t.outerOutline||0,0,20),outerOutlineColor:textColor(t.outerOutlineColor,'#ffffff'),shadow:!!t.shadow,shadowColor:textColor(t.shadowColor,'#000000'),shadowOpacity:clamp(Number.isFinite(+t.shadowOpacity)?+t.shadowOpacity:.72,0,1),shadowBlur:clamp(+t.shadowBlur||0,0,40),shadowX:clamp(Number.isFinite(+t.shadowX)?+t.shadowX:4,-50,50),shadowY:clamp(Number.isFinite(+t.shadowY)?+t.shadowY:6,-50,50),box:['none','dark','light','custom'].includes(t.box)?t.box:'none',boxColor:textColor(t.boxColor,'#000000'),boxOpacity:clamp(Number.isFinite(+t.boxOpacity)?+t.boxOpacity:.62,0,1),boxPadding:clamp(Number.isFinite(+t.boxPadding)?+t.boxPadding:.24,0,1),boxRadius:clamp(Number.isFinite(+t.boxRadius)?+t.boxRadius:.12,0,.5),size:clamp(+t.size||48,8,300),letterSpacing:clamp(+t.letterSpacing||0,-10,50),lineHeight:clamp(+t.lineHeight||1.12,.6,3),x:clamp(Number.isFinite(+t.x)?+t.x:.5,0,1),y:clamp(Number.isFinite(+t.y)?+t.y:.85,0,1),opacity:clamp(Number.isFinite(+t.opacity)?+t.opacity:1,0,1),fadeIn:clamp(Number.isFinite(+t.fadeIn)?+t.fadeIn:+t.fade||0,0,Math.min(120,(end-start)/2)),fadeOut:clamp(Number.isFinite(+t.fadeOut)?+t.fadeOut:+t.fade||0,0,Math.min(120,(end-start)/2)),motion:['none','rise','slide-left','pop'].includes(t.motion)?t.motion:'none',motionDuration:clamp(+t.motionDuration||.4,.05,Math.min(120,(end-start)/2)),start,end}})}
export function sanitize(p){if(p?.version!==1||!Array.isArray(p.clips)||!Array.isArray(p.media)||p.clips.length>200||p.media.length>200)throw Error('対応していないプロジェクト形式です。');for(const c of p.clips){if(c.start!==undefined){if(!Number.isFinite(c.start)||c.start<0||c.start>86400)throw Error('配置時刻が無効です。');c.layer=clamp(Math.round(c.layer||0),0,c.kind==='audio'?3:2)}if(c.gap){c.gap=clamp(c.gap,.01,86400);continue}if(!p.media.some(m=>m.id===c.media))throw Error('素材情報が欠けています。');if(!Number.isFinite(c.in)||!Number.isFinite(c.out)||c.in<0||c.out<=c.in)throw Error('IN / OUT が無効です。');if(c.freezeDuration){c.freezeDuration=clamp(c.freezeDuration,1/30,60);c.freezeAt=clamp(c.freezeAt??c.in,0,p.media.find(m=>m.id===c.media).duration-.00001)}c.speed=clamp(c.speed??1,.05,20);c.endSpeed=clamp(c.endSpeed??c.speed,.05,20);c.motionPreset=['none','push-in','pull-out','pan-left','pan-right','pan-up','pan-down'].includes(c.motionPreset)?c.motionPreset:'none';c.motionAmount=clamp(Number.isFinite(+c.motionAmount)?+c.motionAmount:.12,0,.5);c.color={...colors(),...c.color};c.audio={volume:1,mute:false,fadeIn:0,fadeOut:0,...c.audio};c.curve=c.curve||'constant';if(c.timingBase){const b=c.timingBase;if(!Number.isFinite(b.in)||!Number.isFinite(b.out)||b.out<=b.in||b.in<0)throw Error('速度区間が無効です。');c.timingBase={in:b.in,out:b.out,speed:clamp(b.speed??1,.05,20),endSpeed:clamp(b.endSpeed??b.speed??1,.05,20),curve:b.curve||'constant'}}}if((p.audioClips||[]).length>200)throw Error('音声クリップは200個までです。');const defaults=project(),audioClips=(p.audioClips||[]).map(c=>{if(!p.media.some(m=>m.id===c.media)||!Number.isFinite(c.in)||!Number.isFinite(c.out)||c.in<0||c.out<=c.in)throw Error('音声の範囲が無効です。');return{...c,kind:'audio',linked:c.sourceClip?c.linked!==false:false,speed:clamp(c.speed??1,.05,20),endSpeed:clamp(c.endSpeed??c.speed??1,.05,20),curve:c.curve||'constant',hold:clamp(c.hold||0,0,10),layer:clamp(Math.round(c.layer||0),0,3),start:clamp(c.start||0,0,86400),audio:{volume:1,mute:false,fadeIn:0,fadeOut:0,...c.audio}}});return{...defaults,...p,overlayTracks:Array.from({length:3},(_,i)=>({...defaults.overlayTracks[i],...p.overlayTracks?.[i],hidden:!!p.overlayTracks?.[i]?.hidden})),videoTracks:Array.from({length:3},(_,i)=>({...defaults.videoTracks[i],...p.videoTracks?.[i],hidden:!!p.videoTracks?.[i]?.hidden,volume:clamp(p.videoTracks?.[i]?.volume??1,0,2)})),audioClips,audioTracks:Array.from({length:4},(_,i)=>({...defaults.audioTracks[i],...p.audioTracks?.[i],volume:clamp(p.audioTracks?.[i]?.volume??1,0,2)})),analysis:p.analysis||[],effects:(p.effects||[]).slice(0,20).filter(e=>['flash','black-in','black-out'].includes(e.type)).map(e=>{const duration=clamp(e.duration,1/60,86400);return{...e,layer:clamp(Math.round(e.layer||0),0,2),start:clamp(e.start,0,86400),duration,hold:clamp(e.hold||0,0,Math.max(0,duration-1/60)),strength:clamp(e.strength??1,0,1)}}),texts:sanitizeTexts(p.texts),bgm:p.bgm||{volume:.3}}}
