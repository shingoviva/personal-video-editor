...
export function bindStageTransform({root,getClip,enabled,begin,change,finish,guides,status=()=>{},direction=()=>-1,allowScale=()=>true}){
 ...
 root.addEventListener('pointermove',event=>{if(!drag||event.pointerId!==drag.id)return;const rect=root.getBoundingClientRect(),distance=Math.hypot(event.clientX-drag.x,event.clientY-drag.y);if(!drag.moved&&distance<4)return;if(!drag.moved){drag.moved=true;begin()}const next=centeredTransform(drag.origin,event.clientX-drag.x,event.clientY-drag.y,rect.width,rect.height,8,direction());change(next);guides(next);status(`位置 X ${next.x.toFixed(2)} · Y ${next.y.toFixed(2)}${next.snapX||next.snapY?' · 中央に吸着':''}`);event.preventDefault()},true);
