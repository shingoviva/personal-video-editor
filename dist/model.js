...
export function sanitizeTexts(values=[]){return(Array.isArray(values)?values:[]).slice(0,20).map((t,i)=>{const start=clamp(+t.start||0,0,86400-1/60),end=clamp(Number.isFinite(+t.end)?+t.end:start+3,start+1/60,86400),font=String(t.font||'Sans').replace(/["'`;{}]/g,'').slice(0,80)||'Sans';return{id:String(t.id||`text-${i}`),text:String(t.text??'').slice(0,2000),font,align:['left','center','right'].includes(t.align)?t.align:'center',weight:[400,500,600,700,800].includes(+t.weight)?+t.weight:600,color:/^#[0-9a-f]{6}$/i.test(t.color||'')?t.color:'#ffffff',box:['none','dark','light'].includes(t.box)?t.box:'none',outline:0,size:clamp(+t.size||48,8,300),x:clamp(Number.isFinite(+t.x)?+t.x:.5,0,1),y:clamp(Number.isFinite(+t.y)?+t.y:.85,0,1),opacity:clamp(Number.isFinite(+t.opacity)?+t.opacity:1,0,1),fadeIn:clamp(Number.isFinite(+t.fadeIn)?+t.fadeIn:+t.fade||0,0,3),fadeOut:clamp(Number.isFinite(+t.fadeOut)?+t.fadeOut:+t.fade||0,0,3),motion:['none','rise','slide-left'].includes(t.motion)?t.motion:'none',motionDuration:clamp(+t.motionDuration||.4,.05,2),start,end}})}
