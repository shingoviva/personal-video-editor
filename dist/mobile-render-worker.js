...
async function render(p,files,preview,outputPath){
 ...
 for(const row of rows){check();const c=row.clip,res=resources.get(c.media);let iterator,current,next,pathData,sink;let blendCanvas,blendCtx,stableCanvas,stableCtx,still;
 try{
 if(!c.gap&&res?.imageFile){still=await imageBitmap(res.imageFile,res.metadata,Math.min(8192,Math.max(cfg.width,cfg.height)*(c.scale||1)))}
 if(!c.gap&&!still){if(!res?.video)throw Error('映像トラックがありません。');if(c.stabilization!=='OFF'){progress('手ぶれの動きを解析中',frameIndex/cfg.frames*.9);pathData=await stabilize(res.video,c,value=>progress('手ぶれの動きを解析中 '+Math.round(value*100)+'%',frameIndex/cfg.frames*.9));postMessage({type:'note',text:'端末版の平行移動補正・クロップ '+(pathData.crop*100).toFixed(1)+'%'});}
 ...
 if(still){painter.draw(still,c,p.aspect);ctx.drawImage(processed,0,0)}else if(!c.gap){const source=sourceTime(row,t);while(!next.done&&next.value.timestamp<=source+1e-8){current=next.value;next=await iterator.next()}if(!current)current=next.value;if(!current)throw Error('映像フレームを読み込めません。');blendCtx.globalAlpha=1;blendCtx.drawImage(current.canvas,0,0);if(c.interpolation==='blend'&&!next.done&&next.value.timestamp>current.timestamp){blendCtx.globalAlpha=Math.max(0,Math.min(1,(source-current.timestamp)/(next.value.timestamp-current.timestamp)));blendCtx.drawImage(next.value.canvas,0,0);blendCtx.globalAlpha=1;}
 ...
 }finally{still?.close();await iterator?.return?.();if(blendCanvas){blendCanvas.width=blendCanvas.height=1;}if(stableCanvas){stableCanvas.width=stableCanvas.height=1;}}}
