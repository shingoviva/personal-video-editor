import {sequence,timing,sourceOffset,clamp} from './model.js';
export function outputSettings(p,preview=false){
 ...
 const duration=sequence(p).at(-1)?.end||0;if(!Number.isFinite(duration)||duration<=0)throw Error('書き出し時間が無効です。');
...
export function sourceTime(row,t){const c=row.clip,nodes=(row._timing??=timing(c)).nodes;const dt=Math.max(0,t-row.start);for(let i=1;i<nodes.length;i++){const [x0,y0]=nodes[i-1],[x1,y1]=nodes[i];if(dt<=y1)return Math.min(c.out-1e-6,c.in+x0+(dt-y0)*(x1-x0)/(y1-y0));}return c.out-1e-6;}
...
export function held(row,t){return t-row.start>=timing(row.clip).nodes.at(-1)[1];}
