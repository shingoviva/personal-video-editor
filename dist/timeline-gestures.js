 ...
 for(const el of root.querySelectorAll('[data-clip]')){
  el.onpointerdown=e=>{
   ...
   el.onpointermove=ev=>{
    ...
    if(!moved){begin();if(e.altKey&&!edge&&duplicate){row=duplicate(row)||row;select(row.clip.id,e)}moved=true;el.classList.add('dragging')}
    ...
    let spacing=false;const snapped=t=>{t=Math.round(t/frame)*frame;didSnap=false;if(snap()){let best=targets.reduce((found,a)=>Math.abs(a-t)<Math.abs((found??Infinity)-t)?a:found,undefined);if(best!==undefined&&Math.abs(best-t)<=secondsPerPixel*8){t=best;didSnap=true}}return Math.max(0,t)};
    const c=row.clip;
    if(!edge){
     c.start=snapped(row.start+delta);c.layer=getLayer(ev.clientY)??row.layer;if(snap()&&!didSnap){const result=equalSpacingStart(c.start,timing(c).duration,{...row,layer:c.layer},rows,secondsPerPixel);c.start=result.start;spacing=result.spacing}
    ...
    const now=performance.now();if(now-lastPreview>100){lastPreview=now;preview(c,edge,didSnap,spacing)}
   };
