...
export function textStyle(text={}){return{
 ...
 color:color(text.color),outline:0,
...
export function drawTextCanvas(ctx,text,time,w,h){
 ...
 rows.forEach((line,index)=>{const ly=y+(index-(rows.length-1)/2)*lineHeight;ctx.fillStyle=style.color;ctx.fillText(line,x,ly,maxWidth)});
...
export function rasterizeText(text){
 ...
 const maxWidth=1920*.88,lineHeight=size*1.12,pad=Math.ceil(size*.3),width=Math.max(2,Math.ceil(Math.min(maxWidth,Math.max(...rows.map(line=>probe.measureText(line||' ').width),1))+pad*2)),height=Math.max(2,Math.ceil(rows.length*lineHeight+pad*2));
 ...
 rows.forEach((line,index)=>{const y=height/2+(index-(rows.length-1)/2)*lineHeight;ctx.fillStyle=style.color;ctx.fillText(line,width/2,y,maxWidth)});
...
export class TextPreview{
 ...
 sync(texts){const signature=texts.map(t=>JSON.stringify([t.id,t.text,textStyle(t)])).join('|');if(signature===this.signature)return;this.signature=signature;const wanted=new Set(texts.map(t=>t.id));for(const[id,node]of this.nodes)if(!wanted.has(id)){node.remove();this.nodes.delete(id)}
  for(const text of texts){let node=this.nodes.get(text.id);if(!node){node=document.createElement('div');node.className='text-item';this.root.append(node);this.nodes.set(text.id,node)}const style=textStyle(text);node.textContent=String(text.text??'').slice(0,2000);node.style.fontFamily=fontFamily(style.font);node.style.fontWeight=style.weight;node.style.textAlign=style.align;node.style.color=style.color;node.style.webkitTextStroke='';node.style.background=style.box==='light'?'#f6f7efd1':style.box==='dark'?'#0508069e':'';node.style.padding=style.box==='none'?'0':'.22em .3em';node.style.borderRadius=style.box==='none'?'0':'.12em'}
 }
 update(texts,time,scale){this.sync(texts);for(const text of texts){const node=this.nodes.get(text.id),pose=textPose(text,time),style=textStyle(text);node.hidden=!pose.alpha;if(!pose.alpha)continue;node.style.left=pose.x*100+'%';node.style.top=pose.y*100+'%';const layout=JSON.stringify([text.text,text.size,style,scale,this.root.clientWidth,this.root.clientHeight]);if(node.dataset.layout!==layout){node.dataset.layout=layout;node.style.fontSize=Math.max(1,(+text.size||48)*scale)+'px';node.style.webkitTextStroke='';const fit=Math.min(1,this.root.clientWidth*.92/Math.max(1,node.scrollWidth),this.root.clientHeight*.9/Math.max(1,node.scrollHeight));node.dataset.fit=String(fit)}const base=style.align==='left'?'translate(0,-50%)':style.align==='right'?'translate(-100%,-50%)':'translate(-50%,-50%)';node.style.opacity=pose.alpha;node.style.transform=`${base} scale(${node.dataset.fit||1})`;node.style.transformOrigin=style.align==='left'?'left center':style.align==='right'?'right center':'center'}}
