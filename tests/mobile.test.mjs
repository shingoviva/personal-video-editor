...
import {outputSettings,sourceTime,gainAt,mixWindow} from '../dist/mobile-model.js';
...
const path=smoothPath(Array.from({length:30},(_,i)=>({time:i/12,x:i*.02+(i%2?.015:0),y:0})),'NATURAL');assert.ok(path.scale>1&&path.scale<=1.175);assert.ok(Math.abs(correctionAt(path,1).x)<=.08);assert.equal(safeName('a/b:test','mp4'),'a_b_test.mp4');
