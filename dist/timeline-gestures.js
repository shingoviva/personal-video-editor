  ...
  el.onpointerdown=e=>{
   ...
   el.onpointermove=ev=>{
    ...
    if(!edge){
     ...
     for(const g of companions){g.row.clip.start=g.start+actual;g.row.clip.layer=clamp(g.layer+c.layer-row.layer,0,c.kind==='audio'?3:2);const node=[...root.querySelectorAll('[data-clip]')].find(n=>n.dataset.clip===g.row.clip.id);if(node){node.style.left=g.row.clip.start/Math.max(duration,.001)*100+'%';node.style.top=((g.row.clip.layer-g.layer)*(c.kind==='audio'?44:-52))+'px'}}
     for(const value of others){value.item.start=value.start+actual;if(value.kind==='text')value.item.end=value.item.start+value.span;const node=[...timelineRoot.querySelectorAll('[data-clip],[data-fx],[data-text-chip]')].find(n=>(n.dataset.clip||n.dataset.fx||n.dataset.textChip)===value.item.id);if(node)node.style.left=value.item.start/Math.max(duration,.001)*100+'%'}
